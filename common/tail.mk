...
# Set to 0 for boards that aren't a node in common/Data/2024CAR.dbc and don't
# want the DBC/DTC codegen pipeline run (e.g. bench-only HIL boards). Default
# preserves existing behaviour for every vehicle board.
DBC_CODEGEN ?= 1
...
# MCU_DEFINE can be overridden by board.mk (before including this file) for
# boards using a different specific part in the same architecture family
# (e.g. STM32F769xx instead of the default STM32F767xx for F7).
...
ifeq ($(DBC_CODEGEN), 1)
SRC += $(GEN_SRC_DIR)/$(BOARD_NAME)_can.c
endif
...
$(GEN_SRC_DIR)/$(BOARD_NAME)_can.c: $(CAN_FILES_GEN_SCRIPT) $(DBC_FILE)
	@mkdir -p $(GEN_DIR)
	@$(PYTHON) $(CAN_FILES_GEN_SCRIPT) $(CURR_BOARD) $(BOARD_F0_OR_F7_OR_F4)
