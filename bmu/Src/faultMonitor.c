...
// Disabled for HV power supply testing: no interlock loop waits at startup
// and no HV faults from BOTS, BSPD, TSMS or HW check while running
...
void faultMonitorSendStatusTask(void *pvParameters) {
    ...
    while (1) {
        ...
        // Disabled for HV power supply testing (no battery/EBOX)
