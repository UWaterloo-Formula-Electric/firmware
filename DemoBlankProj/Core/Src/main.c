...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
//debugMsg, not finished, use DEBUG_PRINT instead
#define DEBUG_PRINT(...) \
    do { \
        if (!printQueue) { \
            handleError(); \
...
int main(void)
{
  ...
  /* definition and creation of debugMsgName */
  osThreadDef(debugMsgName, debugMsg, osPriorityLow, 0, 1024);
  debugMsgNameHandle = osThreadCreate(osThread(debugMsgName), NULL);
...
/**
* @brief Function implementing the debugMsgName thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_debugMsg */
__weak void debugMsg(void const * argument)
{
  /* USER CODE BEGIN debugMsg */
  /* Infinite loop */
  for(;;)
  {
    osDelay(1);
  }
  /* USER CODE END debugMsg */
}
